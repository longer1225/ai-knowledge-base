...
class ChatPage(QWidget):
    def __init__(self, main):
        ...
        self.send_btn.setFont(QFont("Segoe UI Symbol", 18))
                ...
                for msg in res.get("data", []):
                    if msg.get("question"):
                        self.chat_layout.insertWidget(
                            ...
                            ChatBubble(msg["question"], True)
                        )
                    if msg.get("answer"):
                        self.chat_layout.insertWidget(
                            self.chat_layout.count() - 1,
                            ChatBubble(msg["answer"], False)
                        )
    ...
    def ask(self):
        ...
        self.current_ai_bubble = ChatBubble("🤔 思考中...", False)
        self.chat_layout.insertWidget(
            self.chat_layout.count() - 1,
    ...
    def process_typewriter(self):
        if self.char_queue:
            ...
            self.current_ai_bubble.label_ref.setText(
                self.current_ai_bubble.full_text + " ▌"
            )
